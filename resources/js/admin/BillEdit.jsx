...
const BillEdit = ({ token }) => {
  ...
  const [preview, setPreview] = useState({
    ...form,
    bill_copy: null, // Initialize as null or empty string for preview
  });
    ...
    const fetchBill = async () => {
      try {
        ...
        const updatedForm = {
          ...
          items: billData.items.length > 0 ? billData.items : [{ item_name: '', quantity: 1, price: 0 }],
          bill_copy: null, // File isn't fetched; only set if user uploads new one
          terms_condition_ids: billData.terms_conditions ? billData.terms_conditions.map(tc => tc.id) : [],
        };
        setForm(updatedForm);
        setPreview({ ...updatedForm, bill_copy: billData.bill_copy ? billData.bill_copy.split('/').pop() : null }); // Use file name from URL
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.bill_copy) formData.append('bill_copy', form.bill_copy); // Only append if new file uploaded
    ...
    <div className="p-4 sm:p-6 md:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <form onSubmit={handleSubmit} className="w-full md:w-1/2 space-y-4 bg-white p-4 rounded-2xl shadow-lg">
          ...
          <button type="submit" className="bg-green-500 text-white p-2 rounded w-full">Update Bill</button>
        </form>
        ...
        <div className="w-full md:w-1/2 bg-white p-4 rounded-2xl shadow-lg">
          ...
          {preview.bill_copy && <p><strong>Bill Copy:</strong> {preview.bill_copy}</p>} {/* Renders file name */}
